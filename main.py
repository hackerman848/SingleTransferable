...
if datetime.now().month == 8 or (datetime.now().month == 9 or datetime.now().day < 10):
	pass
else:
	raise TimeoutError("Demo app out of date.")"""
...
question_tab = tk.StringVar()
question = tk.StringVar()
seats = tk.StringVar()
output_tab = tk.StringVar()
...
try:
	...
	if df.columns[1] == "Unnamed: 1":
		...
		question_tab.set(df.columns[1])
	if df.columns[2] == "Unnamed: 2":
		question.set("")
	else:
		question.set(df.columns[2])
	if df.columns[3] == "Unnamed: 3":
		seats.set("")
	else:
		seats.set(df.columns[3])
	if df.columns[4] == "Unnamed: 4":
		output_tab.set("")
	else:
		output_tab.set(df.columns[4])
...
def quit_program() -> None:
	...
	_input = question_tab.get()
	_quest = question.get()
	_seats = seats.get()
	_output = output_tab.get()
	save_values(_sheet, _input, _quest, _seats, _output)
	root.destroy()
...
def call_calculator() -> None:
	...
	if not _seats.isnumeric() or int(_seats) < 1:
		save_values(_sheet, _input, _quest, "", _output)
		error_msg()
		return

	save_values(_sheet, _input, _quest, _seats, _output)
	start_calc(_sheet, _input, _quest, int(_seats), _output)
...
frame = ttk.Frame(input_frame)
frame.pack(side = "top", fill = "x", expand = True, pady = 4)
ttk.Label(frame, text = "Question's Tab Name: ").pack(side = "left", expand = False)
ttk.Entry(frame, textvariable = question_tab, width = 30).pack(side = "left", expand = True, fill = "x")
...
output_frame = ttk.Frame(root, borderwidth = 3, relief = "solid")
output_frame.pack(side = "top", fill = "both", expand = True)

ttk.Label(output_frame, text = "Outputs").pack(side = "top", expand = False, fill = "y")

frame = ttk.Frame(output_frame)
frame.pack(side = "top", fill = "x", expand = True, pady = 4)
ttk.Label(frame, text = "Output Tab Name: ").pack(side = "left", expand = False)
ttk.Entry(frame, textvariable = output_tab, width = 30).pack(side = "left", expand = True, fill = "x")

ttk.Button(root, text = "Save & Calculate", command = call_calculator).pack(side = "left", pady = (6, 4), padx = (4, 0))
# label = ttk.Label(root, text = "Standby")
# label.pack(side = "left", pady = (6, 4), padx = (0, 4))
